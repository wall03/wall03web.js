...
export async function POST(request: Request) {
	try {
		...
		const existingBlogDataPath = path.join(
			process.cwd(),
			"app",
			"blog",
			"blog-data.json",
		...
		if (existsSync(existingBlogDataPath)) {
			const existingData = JSON.parse(
				await readFile(existingBlogDataPath, "utf-8"),
			);
			existingSlugs = existingData.map((post: any) => post.slug);
		}

		const newSlugs = posts.map((post: any) => post.slug);
		const deletedSlugs = existingSlugs.filter(
			(slug: string) => !newSlugs.includes(slug),
		);
