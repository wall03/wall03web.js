...
export default function EditPosts() {
    ...
    const loadPosts = async () => {
      const postsWithContent = await Promise.all(
        blogCardData.map(async (post) => {
          try {
            const response = await fetch(`/blog-texts/${post.slug}.html`);
  ...
  const deletePost = (index: number) => {
    if (confirm(`Are you sure you want to delete "${posts[index].title}"?`)) {
  ...
  return (
    <div>
      ...
      <p style={{ marginBottom: "1rem" }}>
        Logged in as admin. Edit the posts below and click Submit to save changes.
      </p>
      <button
        onClick={addNewPost}
        style={{
        ...
        >
          ...
          <div style={{ marginBottom: "1rem" }}>
            ...
            <input
              type="text"
              value={post.cover}
              onChange={(e) => updatePost(index, "cover", e.target.value)}
            />
          </div>
      ...
      <div className="inset" style={{ position: "sticky", bottom: "1rem", width: "100%",borderRadius:"20px", backgroundColor: "var(--bg-accent)", padding: "1rem" }}>
        <button
